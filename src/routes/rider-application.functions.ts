...
export const submitRiderApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const required: Array<[unknown, string]> = [
      ...
      [app.lga, "LGA"],
      [app.photo_url, "Passport photograph"],
      [app.vehicle_type, "Vehicle type"],
      [app.plate_number, "Plate number"],
      [app.drivers_license_number, "Driver's license number"],
      [app.drivers_license_front_url, "Driver's license (front)"],
      [app.drivers_license_back_url, "Driver's license (back)"],
      [app.vehicle_insurance_url, "Vehicle insurance"],
