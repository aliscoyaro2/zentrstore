...
/**
 * Server functions backing the accountless merchant application flow.
 *
 ...
 * to read/write `merchant_applications`, which has no client-facing RLS
 ...
 *
 * This mirrors src/lib/rider-application.functions.ts deliberately, so the
...
// ---------------------------------------------------------------------------
// Step 0 — Email OTP (reuses Supabase Auth's own OTP email, the same path
// the rider application and customer signup both use). The session it
// creates is discarded immediately; we only care that the code was correct,
// never that a login persists.
// ---------------------------------------------------------------------------

export const sendMerchantApplicationOtp = createServerFn({ method: "POST" })
...
export const verifyMerchantApplicationOtp = createServerFn({ method: "POST" })
  .inputValidator((data) =>
    z
      .object({
...
// is fine; only `submitMerchantApplication` enforces completeness.
// ---------------------------------------------------------------------------
...
const stepThreeSchema = z.object({
  opening_time: z.string().trim().optional(), // "HH:MM"
  closing_time: z.string().trim().optional(),
  prep_time_mins: z.number().int().min(1).max(240).optional(),
  self_delivery: z.boolean().optional(),
  pos_available: z.boolean().optional(),
  commission_agreement_accepted: z.boolean().optional(),
});
...
export const saveMerchantApplicationStep = createServerFn({ method: "POST" })
  .inputValidator((data) =>
    z
      .object({
...
export const uploadMerchantApplicationDocument = createServerFn({ method: "POST" })
  .inputValidator((data) => {
...
export const submitMerchantApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const required: Array<[unknown, string]> = [
      ...
      [app.category, "Business type"],
      [app.address_text, "Business address"],
      [app.lga, "LGA"],
      [app.phone, "Phone number"],
      [app.bank_name, "Bank name"],
      [app.account_number, "Account number"],
      [app.account_name, "Account name"],
      [app.opening_time, "Opening time"],
      [app.closing_time, "Closing time"],
    ...
    if (!app.commission_agreement_accepted) {
      throw new Error("Please accept the commission agreement to continue.");
    }
...
export const getMerchantApplicationStatus = createServerFn({ method: "POST" })
  .inputValidator((data) => z.object({ applicationToken: z.string().uuid() }).parse(data))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: app, error } = await supabaseAdmin
